...
>
 ...
 <p className='text-sm mb-1' style={{ color:'var(--tx2)' }}>We sent a verification link to</p>
 <p className='font-semibold text-sm mb-5' style={{ color:'var(--tx)' }}>{submittedEmail}</p>
 <p className='text-xs leading-relaxed mb-8' style={{ color:'var(--tx2)' }}>
 Click the link to activate your account.<br />
 Didn&apos;t receive it? Check your spam folder.
 </p>
 <Link href='/auth/login'
 className='inline-flex items-center justify-center font-semibold text-sm rounded-xl h-10 px-6 text-white'
...
>
 ...
 Start shopping the latest tech today
 </p>
 </motion.div>
 ...
 style={{ color: focusedField ==='name' ?'#6366f1' :'var(--auth-label)' }}>
 Full name
 </label>
 <input
 {...register('name')}
 ...
 autoComplete='name'
 className={fieldCx}
 style={{ color:'var(--tx)', background:'var(--auth-input-bg)', border:'1px solid', ...focusStyle('name') }}
 onFocus={() => setFocusedField('name')}
 onBlur={() => setFocusedField(null)}
 />
 {errors.name && (
 <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
 className='text-xs mt-1.5 text-red-500'>{errors.name.message}</motion.p>
 )}
 </motion.div>
...
>
 ...
 {/* submit */}
 <motion.div {...fadeUp(4)} className='pt-1'>
 <button
 ref={btnRef}
 type='submit'
 disabled={loading}
...
>
 ...
 Create account <ChevronRight size={15} strokeWidth={2.5} />
 </span>
 )}
 </motion.span>
 </button>
 ...
 {/* divider + sign in */}
 <motion.div {...fadeUp(5)} className='mt-6 pt-5' style={{ borderTop:'1px solid var(--auth-divider)' }}>
...
>
 ...
 {/* trust badges */}
 <motion.div {...fadeUp(6)} className='mt-5 flex items-center justify-center gap-4'>
 {[
