...
export default function Footer() {
  return (
    <footer className='relative overflow-hidden border-t border-neutral-100 dark:border-white/6 transition-colors duration-400' style={{ background: 'var(--bg)' } as React.CSSProperties}>
      {/* gradient rule */}
      ...
      <div className='relative z-10 mx-auto max-w-300 px-7 py-14 sm:py-16'>
        ...
        >
          ...
          <div className='col-span-2 md:col-span-1'>
            ...
            <p className='text-[13px] leading-relaxed text-neutral-400 dark:text-white/35 max-w-[260px]'>
            ...
            <div className='flex items-center gap-2.5 mt-6'>
              {socials.map((s) => (
                <a
                  ...
                  href={s.href}
                  aria-label={s.label}
                  className='flex h-9 w-9 items-center justify-center rounded-xl bg-neutral-100 dark:bg-white/[0.05] text-neutral-400 dark:text-white/35 transition-all duration-200 hover:bg-[#6366f1]/10 dark:hover:bg-[#6366f1]/[0.15] hover:text-[#6366f1] dark:hover:text-[#818cf8] hover:scale-105'
                >
          ...
          {[
            ...
            <div key={col.title} className='col-span-2 md:col-span-1'>
              <h4 className='text-[13px] font-bold text-neutral-800 dark:text-white/85 mb-4'>
                {col.title}
              </h4>
              <ul className='space-y-2.5'>
                {col.links.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className='group inline-flex items-center gap-1 text-[13px] text-neutral-400 dark:text-white/35 transition-colors duration-200 hover:text-[#6366f1] dark:hover:text-[#818cf8]'
                    >
                      {link.label}
                      <ArrowUpRight
                        size={11}
        ...
        <motion.div
          ...
          className='flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-neutral-100 dark:border-white/[0.06]'
        >
